...
const el = {
  ...
  btnListenSlow: document.getElementById("btn-listen-slow"),
...
// ---------- 音声合成 (読み上げ) ----------
const NORMAL_RATE = 0.92; // ふつうの はやさ
const SLOW_RATE = 0.55; // ゆっくり はやさ

// ブラウザ／OSが用意している音声一覧（自然で聞き取りやすい女性の声を優先して選ぶ）
let cachedVoices = [];

// 優先したい音声名（自然で聞き取りやすい・女性の声のものを上から順に探す）
...
function pickEnglishFemaleVoice() {
  ...
  // 1) 優先リストの名前と完全一致するものを探す
  for (const name of PREFERRED_FEMALE_VOICE_NAMES) {
    const found = enVoices.find((v) => v.name === name);
    if (found) return found;
  }
  // 2) 名前に "female" が含まれるものを探す
  const femaleByName = enVoices.find((v) => /female/i.test(v.name));
  if (femaleByName) return femaleByName;
  // 3) 名前に "male" が含まれない en-US の音声（女性である可能性が高い）
  const nonMaleUS = enVoices.find(
    (v) => /^en-US/i.test(v.lang) && !/\bmale\b/i.test(v.name)
  );
...
function speak(text, rate = NORMAL_RATE) {
  if (!("speechSynthesis" in window)) return;
...
const RECORD_TIMEOUT_MS = 7000; // これ以上たったら自動でマイクを解除する
...
function toggleRecording() {
  ...
  el.recordStatus.textContent = "きいているよ...はなしてね！(7びょういないに)";
  el.feedback.textContent = "";
  el.feedback.className = "feedback";
  ...
  recognition.onerror = (event) => {
    ...
    if (event.error === "not-allowed" || event.error === "service-not-allowed") {
      ...
      el.recordStatus.textContent = "じかんぎれで マイクを とめたよ。もういちど おしてね！";
    } else {
  ...
  try {
    recognition.start();
    // 一定時間はなさなかったら自動でマイクを解除する（聞きっぱなしを防止）
...
el.btnListen.addEventListener("click", () => {
  if (state.currentSentence) speak(state.currentSentence.en, NORMAL_RATE);
});

el.btnListenSlow.addEventListener("click", () => {
  if (state.currentSentence) speak(state.currentSentence.en, SLOW_RATE);
});
